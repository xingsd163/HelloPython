...
productIds = ["CA-NY-99", "CA-FL-88", "TX-IND-77", "MN9-SIN-66", "NJ8-HKG-55", "PA-DEL-44", "FL-NYC-44", "NC-SIN-33",
              "OR-DEN-33", "AZ-LON-22"]
itemIds = ["HYD-65", "HYD-33", "HYD-11", "HYD-1", "HYD-29", "HYD-58", "HYD-87", "HYD-82", "HYD-97", "HYD-6", "HYD-23",
           "HYD-17", "HYD-12", "HYD-89", "HYD-30", "HYD-2"]
...
windowsFiles = ["C:\\opt\\log\\BigDBBook-www1\\access.log", "C:\\opt\\log\\BigDBBook-www2\\access.log",
                "C:\\opt\\log\\BigDBBook-www3\\access.log"]
files = ["/opt/log/BigDBBook-www1/access.log", "/opt/log/BigDBBook-www2/access.log",
         "/opt/log/BigDBBook-www3/access.log"]
...
fourOhFours = ["/product.screen?productId=8675309&", "/stuff/logo.ico?", "/numa/numa.html?", "/search.do?items=vase&",
               "/rush/signals.zip?", "/passwords.pdf?", "/hidden/anna_nicole.html?", "/productscreen.html?t=ou812&",
               "show.do?productId=products&"]
purchaseErrors = ["/cart/error.do?msg=CreditNotAccepted", "/cart/error.do?msg=FormError",
                  "/cart/error.do?msg=NothingInCart", "/cart/error.do?msg=CreditDoesNotMatch",
                  "/cart/error.do?msg=CanNotGetCart"]
    ...
    for file in files:
        ...
        if os.path.exists(dirname):
            if os.path.exists(file):
                ...
                print
                "Creating " + file  # it will get created when data is written later
        else:
            print
            "Creating " + dirname
            os.makedirs(dirname)
...
# ###################################################################################################
def generate_good_logEntry(ipAddress, agent, referer, jsessionId, files, hostName, percentServerErrors,
                           percentUserErrors, eventTime):
    ...
    if referer == "":
        ...
        if action == 'purchase':
            ...
            bytesXferred = str(random.randint(200, 4000))
            timeTaken = str(random.randint(100, 1000))
            event = ipAddress + " - - [" + eventTime.strftime(
                '%d/%b/%Y:%H:%M:%S') + "] \"" + method + " " + uri + " HTTP 1.1\" " + status + " " + bytesXferred + " \"" + referralUri + "\" \"" + agent + "\" " + timeTaken + "\n"
...
def mainLogGenerator():
    ...
    while currentTime < endTime and logEntryCounter < totalLogEntries:
        ...
        jsessionId = "CA" + str(random.randint(0, 10)) + "MO" + str(random.randint(1, 10)) + "AZ" + str(
            random.randint(1, 10)) + "USANA" + str(jSessionNumber)
        ...
        print
        "Total entries for this client = " + str(numberOfEnteriesForThisClient)
        for logEntryNumber in range(numberOfEnteriesForThisClient):
            ...
            if random.randint(1, 100) < htmlErrors and logEntryNumber != 0:
                ...
                l = generate_good_logEntry(clientIP, useragent, refererURI, jsessionId, files, hostName,
                                           percentServerErrors, percentUserErrors, currentTime)
                if l == True:
                    broke_server = True
                else:
                    broke_server = False
...
setupFileSystem()
print
"File System setup completed sucessfully !!!"
mainLogGenerator()
print
"Log generation completed successfully !!!"
print
"Check /opt/log directory !!!"
